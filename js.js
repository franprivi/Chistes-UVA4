document.addEventListener("DOMContentLoaded", () => {
    const chiste = [
        ...
        "Mamá, mamá, los espaguetis se están pegando. ¡Déjalos que se maten!"
    ];

    let currentIndex = 0;

    function displayChiste() {
        document.getElementById("chiste").textContent = chiste[currentIndex];
    }
    ...
    document.querySelector('.buttons').addEventListener('click', (e) => {
        if (e.target.tagName === 'BUTTON') {
            if (e.target.textContent.includes('Anterior')) {
                currentIndex = (currentIndex - 1 + chiste.length) % chiste.length;
            } else if (e.target.textContent.includes('Siguiente')) {
                currentIndex = (currentIndex + 1) % chiste.length;
            }
            displayChiste();
        }
    });
